...
<html lang="en">
	...
	<?php
		...
		require "../../php/employees/empFunc.php";

		if(isset($_POST['search'])){
			$searchQuery = $_POST['search'];
			$employees = searchEmployees($searchQuery);
		}
		else if(isset($_POST['sort']) && $_POST['sort'] !== 'all'){
			$sortQuery = $_POST['sort'];
			$employees = sortEmployees($sortQuery);
		} else if(isset($_POST['sort']) && $_POST['sort'] === 'all'){
			$employees = getEmployeesForTable();
		}
		else{
			$employees = getEmployeesForTable();
		}
            ...
            <div class="wrapper d-flex justify-content-between container-fluid">
              ...
              <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                <div class="navbar-nav">
                  ...
                  <a class="nav-link active" href="#" aria-current="page">Employees</a>
							...
							<form method="post" class="table-actions__search">
								...
								<form method="post" class="table-actions__sort">
									<select name="sort" id="sort" class="input-box">
										...
										<option value="role" <?php echo $sortQuery === 'all' ? 'role' : '' ?>>Role</option>
									</select>
									<button class="button">Sort</button>
								</form>
							</div>

							<table class="table reservations-table users-table">
								<thead>
									<tr>
										<th scope="col"></th>
								...
								<tbody>
									<?php
										foreach($employees as $key=>$employee){
											...
											<tr>
											<td>
												<img class="bungalows-table__img" src="<?php echo "../../".$employee["imagePath"]; ?>" alt="Employee photo" />
											</td>
											<td>
											...
											<td>
												<div class="users-table__address">
													<span class="street">
														<?php echo $employee['employeeStreet'] . " " . $employee['employeeStreetNumber']; ?>
													</span>
													<span class="city"> 
														<?php echo $employee['employeeCity'] . " " . $employee['employeePostalCode']; ?>
													</span>
													<span class="country">
														<?php echo $employee['employeeCountry']; ?>
													</span>
												</div>
											</td>
											<td>
											...
											<td>
												<div class="status status__status status status__active">Active</div>
											</td>
											...
											<td>
												<div class="reservations-table__actions">
													...
													<form class="d-flex align-items-center justify-content-center" method="get">
														<input type="hidden" name="id" value="<?php echo $employee['id']; ?>" />
														<a href="empItem.php?id=<?php echo $employee['id']; ?>">
													...
													<form class="d-flex align-items-center justify-content-center" action="../../php/employees/deleteEmployee.php" method="post">
														<input type="hidden" name="deleteEmployee" value="<?php echo $employee["id"]; ?>" />
